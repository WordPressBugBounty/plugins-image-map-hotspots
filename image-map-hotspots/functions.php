<?php
function imh_6310_link_css_js($hook)
{
   if(
      $hook != 'toplevel_page_imh-6310-image-map-hotspot' && 
      $hook != 'image-annotation_page_imh-6310-image-map-hotspot-import-export' && 
      $hook != 'image-annotation_page_imh-6310-image-map-hotspot-license' && 
      $hook != 'image-annotation_page_imh-6310-image-map-hotspot-use' && 
      $hook != 'image-annotation_page_imh-6310-wpmart-plugins' &&
      $hook != 'image-map-hotspot_page_imh-6310-image-map-hotspot-import-export' &&
      $hook != 'image-map-hotspot_page_imh-6310-image-map-hotspot-use' &&
      $hook != 'image-map-hotspot_page_imh-6310-image-map-hotspot-license' &&
      $hook !='image-map-hotspot_page_imh-6310-wpmart-plugins'
   ) {
   return;
}
...
function imh_6310_get_user_roles(){
...
function imh_6310_extract_data($data)
{
   $array_key = "";
   $array_value = "";
   $i = 0;
   ...
   foreach ($data as $key => $value) {
      if ($i >= 1) {
         ...
         $array_value .= ($key != 'json_data') ? sanitize_text_field($value) : $value;
      }
      $i++;
   }
...
function imh_6310_search_template($ids, $cssData, $output = 0){
   if(!isset($cssData['search_activation']) && $output) return;
	?>
	<div class="imh-6310-search imh-6310-search-<?php echo esc_attr($ids) ?>">
		<div class="imh-6310-search-container imh-6310-search-template-<?php echo esc_attr($ids) ?>">
			<input type="text" name="imh-6310-search-box"  data-imh-6310-template-id='<?php echo esc_attr($ids) ?>' class="imh-6310-search-box" autocomplete="off" placeholder="<?php echo (isset($cssData['search_placeholder']) && $cssData['search_placeholder'] !== '') ? esc_attr($cssData['search_placeholder']) : '' ?>">
   ...
   function imh_6310_get_option($name) {
		global $wpdb;
   ...
   function imh_6310_version_status() {
      ...
      if(!$db_version){
         $wpdb->query("DELETE FROM {$wpdb->prefix}options where option_name='imh_6310_version_info'");
            $wpdb->query("INSERT INTO {$wpdb->prefix}options(option_name, option_value) VALUES ('imh_6310_version_info', '".imh_6310_PLUGIN_CURRENT_VERSION."')");
      }
      else{
         $key = imh_6310_get_option('imh_6310_license_key');
         if($db_version != imh_6310_PLUGIN_CURRENT_VERSION && $key){
            imh_6310_check_license($key, true);
            $wpdb->query("UPDATE {$wpdb->prefix}options set 
                        option_value='". imh_6310_PLUGIN_CURRENT_VERSION ."' 
                        where option_name = 'imh_6310_version_info'");
         }	
      }
   } 
   ...
   function imh_6310_extract_item($ids)
   {
      global $wpdb;
      $item_table = $wpdb->prefix . 'imh_6310_item';
      $results = array();
      if($ids) {
         $ids = explode(',', $ids);
         if($ids) {
...
function imh_6310_display_icon($ids, $type, $icons, $hover_icons, $image, $hover_image) {
   echo "
      <div class='imh-6310-template-".esc_attr($ids)."-icon-wrapper'>
         <div class='imh-6310-template-".esc_attr($ids)."-icon'>";
         if($type == 1) {
            $hover_icons = $hover_icons ? $hover_icons : $icons;
            echo "
                  <i class='".esc_attr($icons)." imh-6310-icon'></i>
                  <i class='".esc_attr($hover_icons)." imh-6310-hover-icon'></i>
               ";
         } else{
            $hover_image = $hover_image ? $hover_image : $image;
            echo "
                  <img src='".esc_attr($image)."' class='imh-6310-image'>
                  <img src='".esc_attr($hover_image)."' class='imh-6310-hover-image'>
               ";
         }
   echo  "</div>
      </div>
...
function imh_6310_display_read_more($ids, $target, $text, $url, $output = 1) {
   if(!strlen($text)) return;
   if(!$output) return;
   $target = $target ? "target='_blank'" : '';
   echo "
   <div class='imh-6310-template-".esc_attr($ids)."-read-more'>
      <a href='{$url}' {$target}>{$text}</a>
   </div>
   ";
}

function imh_6310_add_new_media($id, $results = [])
   {
      global $wpdb;
      wp_enqueue_media();
      $style_table = $wpdb->prefix . 'imh_6310_style';
      $item_table = $wpdb->prefix . 'imh_6310_item';
?>
      <div class="imh_6310_add_media">
         <h6>Shortcode</h6>
         <div class="imh_6310_add_media_body_shortcode">
            <input type="text" onclick="this.setSelectionRange(0, this.value.length)" value='[imh_6310_image_map id="<?php echo esc_attr($id); ?>"]' />
         </div>
      </div>
      <br />
      <div class="imh_6310_add_media">
         <h6>Customize Items</h6>
         <div class="imh_6310_add_media_body" id="imh-6310-add-or-edit-items">
            <i class="fas fa-plus-circle imh_6310_add_media_add_new_icon"></i>
            Add/Edit Items
         </div>
      </div>
      <br />
      <div class="imh_6310_add_media">
         ...
         <div class="imh_6310_add_media_body" id="imh-6310-rearrange-items">
            <i class="fas fa-user-cog imh_6310_add_media_add_new_icon"></i>
         </div>
      </div>
      <br />
      <div class="imh_6310_add_media">
         <h6>How to Use</h6>
         <div class="imh_6310_add_media_body">
...
<?php
   ...
   function imh_6310_description($ids, $description, $output = 1){
      if(!$description) return;
      if(!$output) return;
      echo "<div class='imh-6310-template-".esc_attr($ids)."-description'>".esc_attr($description)."</div>";
   }

   function imh_6310_export_full_map_hotspot_plugin() {
