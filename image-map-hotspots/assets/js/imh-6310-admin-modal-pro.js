...
jQuery(document).ready(function () {
  ...
  jQuery("body").on("click", ".imh-6310-add-point", function () {
    ...
    jQuery(
      `#imh-6310-add-point .imh-6310-tooltip-img:first-child, 
      #imh-6310-add-point .toggle-tabs li:first-child, 
      #imh-6310-add-point .imh-6310_icon_type:first-child, 
      #imh-6310-add-point .imh-6310_blink_type:last-child,
      #imh-6310-add-point .6310_always_show:last-child,
      #imh-6310-add-point .imh-6310-section-select:first,
  ...
  jQuery("body").on("click", ".imh-6310-update-place-save", function () {
    let iconType = Number(
      jQuery("#imh-6310-edit-point .imh-6310_icon_type:checked").val()
    );
    ...
    if (iconType == 1) {
      ...
      imagePointer = `imh-6310-point-icons-${window.currentPoint}`;
      iconImage = `
        <img src='${mainImg}' class="imh-6310-pin-main-img imh-6310-blinking-${window.currentPoint}" />
        <img src='${hoverImg}' class="imh-6310-pin-hover-img imh-6310-blinking-hover-${window.currentPoint}" />
      `;
    } else {
      ...
      iconImage = `
      <div class='imh-6310-customtext imh-6310-blinking-${window.currentPoint}'>${ponterText}</div>   
      `;
    }
    ...
    ).removeClass(`imh-6310-point-icons-${window.currentPoint}`);
    if (imagePointer) {
      jQuery(
        `.imh-6310-drag[data-id='${window.currentPoint}'] .imh-6310-point-icons`
      ).addClass(imagePointer);
    }
    ...
    @keyframes pulse-animation-${window.currentPoint} {
      ...
      100% {
        ...
        jQuery(`<style type='text/css'>
        .imh-6310-point-icons-${window.currentPoint}::after {
          content: '';
          position: absolute;
          width: calc(${jsonObj[0].imgOrIconSize}px + 10px) !important;
          height:calc(${jsonObj[0].imgOrIconSize}px + 10px) !important;         
          box-shadow: 0 0 1px 3px white;
          animation: pulse-animation-${window.currentPoint} 2s infinite;
          display: block;  
          border-radius: 50%;                
        ...
        jQuery(`<style type='text/css'>
        .imh-6310-blinking-${window.currentPoint} {
          animation: pulse-animation-${window.currentPoint} 2s infinite;         
        }
...
function imh_6310_tooltip_fields() {
  const field = jQuery('select[name="tooltip_position"]');
  var value = Number(field.val());
  imh_6310_toggle_fields(value);
...
function imh_6310_toggle_fields(value) {
  const field = jQuery('select[name="tooltip_position"]');
  if (value && value !== 2 && value !== 5) {
    jQuery(".tooltip-attribute").show();
    let selectedText = field.find("option:selected").text().split(" ");
    jQuery(".tooltip-attribute-text-1").text("Space From " + selectedText[0]);
    jQuery(".tooltip-attribute-text-2").text("Space From " + selectedText[1]);
  } else {
